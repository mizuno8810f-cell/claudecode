...
`docs/json-spec.md` の仕様に従った、JSON駆動の脱出ゲームエンジン実装。
部屋・オブジェクト・イベントの追加は `src/data/game.json` の編集のみで行い、
スキーマ自体 (`src/engine/types.ts`) は変更しない。

## アーキテクチャ

- `src/engine/types.ts` — JSON仕様に対応する型定義（Game / Stage / Room / GameObject / Trigger / Condition / Event / Item）
...
- `src/engine/GameEngine.ts` — ゲーム進行ロジック本体。Trigger/Eventの直列実行・ロック制御・watchStateの状態変化監視（false→true遷移でのみ発火するエッジトリガー）・ナビゲーションスタック・インベントリ・グローバル状態を管理する
